#ifndef __MTD_CPU_CAN3_MCP2515_H__
#define __MTD_CPU_CAN3_MCP2515_H__
...
void MTDCPU_CAN3_MCP2515_Reset(void); 			//发送复位指令，复位mcp2515
unsigned char MTDCPU_CAN3_MCP2515_Read_Register(const unsigned char Addr);	/*读取一个寄存器的值*/
void MTDCPU_CAN3_MCP2515_Read_RegisterS(const unsigned char Addr, unsigned char RegValues[], const unsigned char RegNum);  /*读取多个寄存器的值*/
void MTDCPU_CAN3_MCP2515_Set_Register(const unsigned char Addr,const unsigned char Data); //设置一个寄存器的值
...
void MTDCPU_CAN3_MCP2515_Register_BitModify(const unsigned char Addr,const unsigned char Mask, unsigned char Data); //修改寄存器某位的值
unsigned char MTDCPU_CAN3_MCP2515_Read_Status(unsigned char cmd);		//读取寄存器状态
unsigned char MTDCPU_CAN3_MCP2515_ReadRx_Status(void);	 //读取接受的状态
unsigned char MTDCPU_CAN3_MCP2515_SetControlRegister_Mode(const unsigned char NewMode);		//设置控制寄存器 的工作模式
unsigned char MTDCPU_CAN3_MCP2515_Config_Rate(const unsigned int  BaudRate);							//配置mcp2515 的波特率
void MTDCPU_CAN3_MCP2515_Read_CAN_ID(const unsigned char RxIdReg_Addr, unsigned char* IDE_ExtFlag, unsigned int* Std_id,unsigned int * Ext_id);	//读取接收到的canid标识
void MTDCPU_CAN3_MCP2515_Read_CAN_Message(const unsigned char RxBuf_SIDH_Addr, CanRxMsg* Message);			//读取接受的can消息数据
void MTDCPU_CAN3_MCP2515_Write_CAN_ID(const unsigned char TxIdReg_Addr, const unsigned char IDE_ExtFlag, const unsigned int can_id);		//写入要发送的canid
void MTDCPU_CAN3_MCP2515_Write_CAN_Message(const unsigned char TxBuf_SIDH_Addr, const CanTxMsg* Message);		//写入要发送的can消息
void MTDCPU_CAN3_MCP2515_Start_Transimt(const unsigned char TxBuf_SIDH_Addr);				//开始发送消息
unsigned char MTDCPU_CAN3_MCP2515_Get_Free_TxBuf(unsigned char * TxBufNum);
void MTDCPU_CAN3_MCP2515_Init_CAN_Buffer(void);//设置屏蔽寄存器，滤波寄存器，初始化发送缓冲区
...
#endif  //__MTD_CPU_CAN3_MCP2515_H__
