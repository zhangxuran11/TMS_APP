...
/*
	全局运行时间，单位1ms
	最长可以表示 若32位的即24.85天，如果你的产品连续运行时间超过这个数，则必须考虑溢出问题
*/
...
*********************************************************************************************************
*/
void bsp_InitTimer(void)				//这个函数必须调用时钟节拍的初始化函数。故在delay.c函数后面即可
{
	uint8_t i;
    ...
    */
//	SysTick_Config(SystemCoreClock / 1000);			在delay_init()函数中已经初始化滴答时钟为1ms中断一次。故不需要再调用
...
//	g_iRunTime++;
//	
//	if (g_iRunTime == 0x7FFFFFFFFFFFFFFF)	/* 这个变量是 int64_t 类型，最大数为 0x7FFFFFFF */
//	{
//		g_iRunTime = 0;
//	}
...
void bsp_StartTimer(uint8_t _id, uint32_t _period)
{
	...
	if (_id >= TMR_COUNT)
	{
 	...
 	CPU_CRITICAL_ENTER();		/* 关中断 */		//会关闭全局中断，影响滴答定时器的值
//	OS_CRITICAL_ENTER();		//用调度去枷锁的方式，保证以下代码不被打断
...
///*		//这个功能uCOSiii已经提供了OSTimeGet() 函数，会获取系统当前的节拍，由这个全局变量OSTickCtr;滴答计数器
//*********************************************************************************************************
