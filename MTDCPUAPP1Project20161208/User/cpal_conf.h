/**
  ******************************************************************************
  * @file    Project/STM32_CPAL_Examples/STM32_CPAL_I2C/Basic_EEPROM/cpal_conf.h 
...
------ After configuring CPAL Firmware Library, you should follow these steps to use the Firmware correctly :
      ...
      ==> Code source overload of CPAL Library is reduced when Listen mode is used (when Master mode is disabled code source will decrease significantly).
...
/*   -- Section 1 :                   **** I2Cx Device Selection ****
  ...
  #define CPAL_USE_I2C2          /*<! Uncomment to use I2C1 device */		//WHH，表示使用哪个i2c设备
#endif
/* This define must be uncommented for this example to allow CPAL library the use of I2C1 device when using
   STM3210C-EVAL (Connectivity line) or STM32L152-EVAL (STM32L1xx Ultra Low Power Medium-Density) or STM32L152D-EVAL 
   (STM32L1xx Ultra Low Power High-Density)or STM322xG-EVAL or STM324xG-EVAL evaluation boards */
...
/* This define must be uncommented for this example. In this example I2C1 device 
   is used as master to communicate with Slave components (EEPROM)*/
...
/* This define must be uncommented when DMA programming Model is selected to communicate with EEPROM.
   To select DMA programming Model refer to stm32_eval_i2c_ee_cpal.h file */
...
/* This define must be uncommented when DMA programming Model is selected to communicate with EEPROM and I2C1 Device 
   will receive one byte from EEPROM */

/* Enable the use of IT Programming Model */
//#define CPAL_I2C_IT_PROGMODEL
/* This define must be uncommented when IT programming Model is selected to communicate with EEPROM.
...
/*
W H H
EEPROM函数中的cpal库的时候用到了systick滴答定时器在库cpal_i2c.c中断屏蔽滴答定时器的初始化，以及中断。
可以使用一个基本定时器(TIM7)替代 cpal库中的超时中断功能。
*/
#include "stm324xg_eval.h"		//whh包含有TIM7的定义

#ifndef CPAL_I2C_TIMEOUT_Manager
   void CPAL_I2C_TIMEOUT_Manager(void);
#else   
   void SysTick_Handler(void);  
	 void CPAL_I2C_TIMEOUT_Manager(void);		//W  H H 
