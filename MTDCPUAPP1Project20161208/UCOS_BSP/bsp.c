...
/*下面放置一些外设初始化函数*/	 
	...
	MTD_CPU_CANLED_Gpio_Init(); //canled指示灯的初始化
	...
	/*RTC初始化*/
	ISL1208_Init();
	//	Auto_Time_Set();	//上电的时候RTC并不配置时间，要不RTC时间就会变化，RTC本来就在运行	
	...
	Check_CPU_Id();	
		...
		/*17.can adder 设置 来区分每个板子*/
	MTDCPU_CanAdd_Gpio_Init();  //初始化can add 旋钮GPIO
	ChooseBoard	= MTDCPU_CanAddVal();
