...
#include "CRC_CCITT.h"
/*
代码中crcInit() 函数用来计算crcTable，因此在调用 crcCompute 前必须先调用 crcInit()。
不过，对于嵌入式系统，RAM是很紧张的，最好将 crcTable 提前算好，作为常量数据存到程序存储区而不占用RAM空间。
*/
 ...
 */
//void crcInit(void)				//初始化，将数据计算到crcTable 这个大数组中，
//{
//    width_t remainder;
//    width_t dividend;
//    int bit;
...
width_t crcCompute(unsigned char * message, unsigned int nBytes)
{
    ...
    return (remainder ^ FINAL_XOR_VALUE);					// W H H 这个crc结果值 异或的 0x0000;没有取反
} /* crcCompute() */
