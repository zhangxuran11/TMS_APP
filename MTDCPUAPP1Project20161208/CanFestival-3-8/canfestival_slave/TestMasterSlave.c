...
void InitNodem(CO_Data* d, UNS32 id)
{
...
/***************************  MAIN  *****************************************/
/****************************************************************************/
//int main(int argc,char **argv)		//linux下使用
//{
...
{
		initTimer();   //初始化定时器
//		InitNodem(&TestMaster_Data, 0x1);
//		canInit(CAN2,CAN_BAUD_1M);
	
		TestMaster_Data.canHandle	=	CAN2;
		TestMaster_Data.heartbeatError = TestMaster_heartbeatError;
		TestMaster_Data.initialisation = TestMaster_initialisation;
		TestMaster_Data.preOperational = TestMaster_preOperational;
		TestMaster_Data.operational = TestMaster_operational;
		...
		TestMaster_Data.post_SlaveStateChange = Master_post_SlaveStateChange;		// W H H 
//		if(!canOpen(&MasterBoard,&TestMaster_Data)){
//			eprintf("Cannot open Master Board (%s,%s)\n",MasterBoard.busname, MasterBoard.baudrate);
//			goto fail_master;
//		}
