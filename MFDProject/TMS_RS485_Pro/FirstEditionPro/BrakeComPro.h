...
typedef struct _BRAKE_SDR_TEXT		//请求数据文本
{
	...
	u8 ClearDataAndSetTime;	//清除数据 和 设置时间
	/*
	u8 ClearStateDataBit 	:1; //清除状态数据＊2  清除：１该字节保持约 1 秒（相当于传送周期 5）时，删除 BCU 内部所有的状态数据，记录件数为 0
	u8 TimeSetBit 				:1; //时间设定＊３  时间设定：1 该字节保持约 1 秒（相当于传送周期 5）时，基于启动时的时钟信息 Text １～６设定制动控制装置内部的时钟
	u8 unUseBit 					:6; //没有使用的位
	*/
