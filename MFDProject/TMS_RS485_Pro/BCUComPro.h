...
#define	 BCU_ACK						0x06
#define  BCU_NACK		  			0x15

#define	 BCU_RxSD						0x01
#define  BCU_RxTD						0x02

/*定义错误编码值*/
#define  BCU_DataError  				-1			//接受的前4字节有不一致的错误
#define  BCU_NACKError					-2			//表示接受到NACK消息
#define  BCU_FrameError					-3			//检测到帧错误、奇偶校验错误时
#define  BCU_BCCError       	  -4  	 	//BCC 符号不符合计划值时
#define  BCU_GetError						-5			//表示接受数据错误

#define  BCU_GetTIM1OutError	  -6			//发送请求之后，15ms以内没有接收到数据
#define  BCU_GetTIM2OutError	  -7			//发送请求之后，10ms以内没有接收到数据
#define  BCU_GetTIM3OutError	  -8			//发送请求之后，10ms以内没有接收到数据
#define  BCU_GetTIM4OutError	  -9			//发送请求之后，10ms以内没有接收到数据
#define  BCU_GetTIM5OutError	  -10			//发送请求之后，10ms以内没有接收到数据
...
//	u8 mode :接受的数据为SD，或者 TD0,TD1
	u8 GetNum ：1表示第一次 获取数据， 2表示错误发送NACK 再次获取数据
返回值：0,无错，  其他有错。
*/
