...
static u8  fac_us=0;							//us延时倍乘数  (1us需要多少节拍数)			   
static u16 fac_ms=0;							//ms延时倍乘数,在os下,代表每个节拍的ms数   (1ms需要多少节拍数)
...
//在本系统中不会调用SysTick_Handler(),中断函数，因为在启动文件中定义的滴答定时器的中断服务程序是 OS_CPU_PendSVHandler
//故 滴答定时器调用的是 OS_CPU_PendSVHandler()函数的中断服务
//最新2016-05-12: 修改了启动文件，故中断函数将会调用延时函数中的SysTick_Handler()中断服务程序
...
{	
    ...
    CPU_CRITICAL_EXIT();
		...
		BSP_Idle();								//可以放入到滴答定时器的中断函数中去	 //W H H  
	//		SysTick_ISR();						//软件定时器的中断服务程序  W H H  
...
void delay_init(u8 SYSCLK)
{
	...
	SysTick->LOAD=reload-1; 									//每1/OS_TICKS_PER_SEC 秒中断一次	
...
/*
//W H H 微妙级别的延时并不好
实际通过示波器测试，微妙延迟函数比实际设置实际多运行1.9us左右的时间。
这个时间不准确主要由于 开关任务调度器。所加的时间比较长。
*/
#if SYSTEM_SUPPORT_OS 						//如果需要支持OS.
//延时nus
//nus:要延时的us数.	
...
{		
	//方法一： 时间速度us的延时  每次相差1.99us。
	u32 ticks,reload,tcnt;
	u32 told,tnow;
...
//	/*方法二整体，加入任务调度器关闭 开启   时间速度us的延时  每次相差1.96us。*/
