...
//	uart3_init();		//uart3的GPIO初始化
//	bsp_InitUart(); 	  /* uart1 初始化串口 */
	Uart1_init();
...
//	LEDGpio_Init();				//LED的GPIO初始化
//	KEYGpio_Init();				//按键GPIO初始化
		
	RunLedGpio_Init();			//初始化运行灯
//	Buzzer_Func_Init();		 //初始化蜂鸣器

	CCULED_LEDGpio_Init();  //26个状态灯的初始化
...
//TIM5_Int_Init(5000-1,8400-1);			//定时器5主要控制RUNLED灯的控制，需要放在led灯初始化的前面。作为字库更新的提示灯。
	...
	/*11.SD卡上电初始化，并且显示SD卡的容量*/		//SD卡中断的配置在sd卡文件已经写入	,要修改sd卡的中断，在sd的文件中修改、/*SD卡.c文件已经配置了中断优先级*/
//	{
//		u8 i;
//		int res;
//		SD_Error Status;
...
//			if(Status==SD_OK) 
//			{
//				printf("SD卡初始化success,存在!\r\n");  
//				break;
//			}			
//		}while(i<=10);
//		if(Status!=SD_OK)
//		{
//			printf("SD卡初始化error,不存在!\r\n");
//		}
//		SD_ShowVolume("0:");		//显示SD卡的容量
//		
