            ...
            if ( nodeId == ConsumerHeartBeat_nodeId )
              {
                ...
                d->ConsumerHeartBeatTimers[index] = SetAlarm(d, index, &ConsumerHeartbeatAlarm, MS_TO_TIMEVAL(time), 0);
								...
								/*W H H 将全局表示 接受到心跳的标志置于 1*/
								/*W H H can1接受状态的改变*/
								{
									if(d->NMTable[nodeId] == Operational)  //whh 如果节点状态为0x5，操作状态。
									{
										if(g_RxCAN1MesStateFlag[nodeId] == RxCanNodeGuardMesState)/* &&(g_RxCAN1HeartStateFlag[nodeId] == InOperatState)*/
										{
											g_RxCAN1MesStateFlag[nodeId] = NoRxCanMesState;
											g_RxCAN1HeartFlag[nodeId] = RxHeart; 		//W H H,表示接收到了心跳
										}
										
										else if(g_RxCAN2MesStateFlag[nodeId] == RxCanNodeGuardMesState)/* && (g_RxCAN2HeartStateFlag[nodeId] == InOperatState)*/
										{
											g_RxCAN2MesStateFlag[nodeId] = NoRxCanMesState;
											g_RxCAN2HeartFlag[nodeId] = RxHeart; 		//W H H,表示接收到了心跳
										}
									}
								}
...
//			printf("chanle=0x%x\r\n",d->canHandle);										//W H H，这里没有错，可以删除printf
      canSend(d->canHandle,&msg );
