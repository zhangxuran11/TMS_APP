...
void SendPIDS_SDR(S_TMS_PIDS_SDR_Text * pInData,u8 * pOutData)
{
	...
	memcpy(&DataFrame.DataText,pInData,sizeof(S_TMS_PIDS_SDR_Text));	 //将发送的文本信息全部拷贝给数据帧
	...
	CrcValue = 	(crcCompute((u8 *)&DataFrame.AddressL8,sizeof(S_TMS_PIDS_SDR_Text)+3));		//计算得到crc值
...
//	u8 *pDireOutData;			//定义一个指针，指向输出的数据 pOutData;
	static u8 s_PIDSRxErrCount = 0;							 //静态局部变量。检测到连续5 次接收失败时，则判定为传输异常 
...
//	pDireOutData = pOutData;
	...
	s_PIDSRxErrCount = 0;		//将不连续的标志清零
	
	while(*pDireInData == PIDS_PAD1)			//计算有几个字节是0x7E
	{
		i++;
		pDireInData++;
	}
...
s8 CheckPIDSGetError(u8 *pInData)
{
	...
	{
		#if DEBUG_RS485==1
			printf("Rs485 Check Frame Error,Check Code Error!\r\n");
		#endif
...
//	{
//		#if DEBUG_RS485==1
//			printf("Rs485 Accept the start character, the address, the command has a non-conformance!\r\n");
//		#endif		
