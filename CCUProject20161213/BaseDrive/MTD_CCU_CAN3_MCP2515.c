...
extern OS_TCB CAN3MasterRxTaskTCB;
...
void	MCP2515_INT1_GpioInit(void)
{
  ...
  EXTI_InitStructure.EXTI_Line = EXTI_Line7;                   //配置中断线为中断线11
    ...
    if(EXTI_GetITStatus(EXTI_Line7) != RESET)
		{
			...
			if(RxMessage.DLC >0)
			{
				...
				OSTaskSemPost(&CAN3MasterRxTaskTCB,OS_OPT_POST_NONE,&err);
			}
...
{  
    ...
    */   
	...
	//设置波特率
	res = MTDCCU_CAN3_MCP2515_Config_Rate(CanBaudRate);		//设置波特率
	if(res == MCP2515_OK)
	{
		MTDCCU_CAN3_MCP2515_Init_CAN_Buffer();  //清空屏蔽，滤波，发送，接受寄存器
		
		// enable both receive-buffers to receive messages
		// with std. and ext. identifiers
		// and enable rollover
		MTDCCU_CAN3_MCP2515_Register_BitModify(RXB0CTRL,RXM /*| BUKT*/,/*RXM_VALID_ALL*/RXM_RCV_ALL /*| BUKT_ROLLOVER*/);	//设置接受控制寄存器0,接受模式00，接受滚存
		MTDCCU_CAN3_MCP2515_Register_BitModify(RXB1CTRL,RXM,/*RXM_VALID_ALL*/ RXM_RCV_ALL);								//设置接受控制寄存器1，接受所有id
		//	MTDCCU_CAN3_MCP2515_Set_Register(RXB0CTRL,0x00);// 输入缓冲器0控制寄存器, 接收所有符合滤波条件的报文，滚存禁止
		//	MTDCCU_CAN3_MCP2515_Set_Register(RXB1CTRL,0x00);
		
		
		//配置中断
		/*错误标志寄存器 不能使能，会导致进入错误中断*/
		//MTDCCU_CAN3_MCP2515_Register_BitModify(CANINTE,MERRE | ERRIE | RX0IE /*| RX1IE*/,IVRE_ENABLED|ERRIE_ENABLED |RX0IE_ENABLED /*G_RXIE_ENABLED*/);  //0,1全部接受中断使能  //使能ERRIE：错误中断使能位\报文接收中断RX0IE\RX1IE
		...
		//配置相关引脚
		MTDCCU_CAN3_MCP2515_Set_Register(TXRTSCTRL,0x00);	// TXnRST作为数字引脚，非发送请求引脚
		MTDCCU_CAN3_MCP2515_Set_Register(BFPCTRL,0x00);		// RXnRST禁止输出
	}
	...
	//	res= MTDCCU_CAN3_MCP2515_Read_Register(CNF2);	//读取波特率寄存器
...
void MTDCCU_CAN3_MCP2515_Reset(void)	     //MCP2515器件复位
{
   ...
   MTDCCU_SPI1_SendByte(SPICAN_RESET);			//发送寄存器复位命令		//0xC0
   MTDCCU_SPI1_CS_HIGH();										/* 片选口线置高不选中 */
	 delay_ms(10);
}
...
unsigned char MTDCCU_CAN3_MCP2515_Read_Register(const unsigned char Addr)
{
    ...
    MTDCCU_SPI1_CS_LOW();                //置MCP2515的CS为低电平
    MTDCCU_SPI1_SendByte(SPICAN_READ);   //发送读命令	//0x03
    MTDCCU_SPI1_SendByte(Addr); 				 //发送地址	
    Data = MTDCCU_SPI1_ReadByte();   		 //读一字节 
    MTDCCU_SPI1_CS_HIGH();               //置MCP2515的CS为高电平 
    return Data; 
}
...
void MTDCCU_CAN3_MCP2515_Read_RegisterS(const unsigned char Addr, unsigned char RegValues[], const unsigned char RegNum)
{
	  ...
	  MTDCCU_SPI1_CS_LOW();                //置MCP2515的CS为低电平
		MTDCCU_SPI1_SendByte(SPICAN_READ);   //发送读命令
    MTDCCU_SPI1_SendByte(Addr); 				 //发送地址	
	
		// mcp2515 has auto-increment of address-pointer	
		for(i=0;i<RegNum;i++)
		{
			RegValues[i]=MTDCCU_SPI1_ReadByte();	//读取数据
		}
		MTDCCU_SPI1_CS_HIGH();               //置MCP2515的CS为高电平 
}
...
{
	  MTDCCU_SPI1_CS_LOW();   						//使能片选     
    MTDCCU_SPI1_SendByte(SPICAN_WRITE); //发送写命令		//0x02
    MTDCCU_SPI1_SendByte(Addr); 				//发送地址				
    MTDCCU_SPI1_SendByte(Data); 				//写入数据
    MTDCCU_SPI1_CS_HIGH(); 							//关闭片选
}
...
//		unsigned char i;
//		MTDCCU_SPI1_CS_LOW();   						//使能片选
//	  MTDCCU_SPI1_SendByte(SPICAN_WRITE); //发送写命令
//    MTDCCU_SPI1_SendByte(Addr); 				//发送地址	
//	
//		// mcp2515 has auto-increment of address-pointer
//		for(i=0;i<RegNum;i++)
//		{
//			MTDCCU_SPI1_SendByte(SetValues[i]);//写入数据
//		}
//    MTDCCU_SPI1_CS_HIGH(); 							//关闭片选		
...
{ 
    MTDCCU_SPI1_CS_LOW();                			//使能片选
    MTDCCU_SPI1_SendByte(SPICAN_BIT_MODIFY); 	//发送位修改命令			//0x05
    MTDCCU_SPI1_SendByte(Addr); 							//发送地址
    MTDCCU_SPI1_SendByte(Mask); 							//发送掩码
    MTDCCU_SPI1_SendByte(Data); 							//写入数据
    MTDCCU_SPI1_CS_HIGH();    								//关闭片选
}
...
unsigned char MTDCCU_CAN3_MCP2515_Read_Status(unsigned char cmd)
{
		unsigned char State;
		MTDCCU_SPI1_CS_LOW();                				//使能片选
		MTDCCU_SPI1_SendByte(/*SPICAN_READ_STATUS*/cmd); 	//发送读状态命令
		State = MTDCCU_SPI1_ReadByte();							//读取状态
		MTDCCU_SPI1_CS_HIGH();											//关闭片选
...
unsigned char MTDCCU_CAN3_MCP2515_ReadRx_Status(void)
{
		...
		MTDCCU_SPI1_CS_LOW();                				//使能片选
		MTDCCU_SPI1_SendByte(SPICAN_RX_STATUS); 		//发送读状态命令
		RxState = MTDCCU_SPI1_ReadByte();						//读取状态
		MTDCCU_SPI1_CS_HIGH();											//关闭片选
...
{
		unsigned char mode;
		MTDCCU_CAN3_MCP2515_Register_BitModify(CANCTRL,REQOP,NewMode);	//修改模式
...
//		canctrl = MTDCCU_CAN3_MCP2515_Read_Register(CANCTRL);			//读出当前状态		whh
//		MTDCCU_CAN3_MCP2515_SetControlRegister_Mode(REQOP_CONFIG);//修改模式为 配置模式、	whh
//		
//		MTDCCU_CAN3_MCP2515_Set_Register(CNF1,((sjw-1) << 6) | brp);		//设置波特率
//		MTDCCU_CAN3_MCP2515_Set_Register(CNF2,BTLMODE | ((ps1-1) << 3) | (propseg-1));
//		MTDCCU_CAN3_MCP2515_Register_BitModify(CNF3,PHSEG2, (ps2-1));
//		
//		MTDCCU_CAN3_MCP2515_Set_Register(CANCTRL,canctrl);	//返回为之前的模式		whh
//		
//		return MCP2515_OK;
//	}
...
void MTDCCU_CAN3_MCP2515_Read_CAN_Message(const unsigned char RxBuf_SIDH_Addr, CanRxMsg* Message)
{
		...
		memset(Message,0,sizeof(CanRxMsg));	//清空接受缓冲区
		/*获得id*/
		MTDCCU_CAN3_MCP2515_Read_CAN_ID(RxReg_Addr,&(Message->IDE),&(Message->StdId),&(Message->ExtId));	//得到id
		
		RxControlReg_Value = MTDCCU_CAN3_MCP2515_Read_Register(RxReg_Addr-1);		//得多接受控制寄存器值
	
		Message->DLC = MTDCCU_CAN3_MCP2515_Read_Register(RxReg_Addr+4);					//读取数据长度
		...
		MTDCCU_CAN3_MCP2515_Read_RegisterS(RxReg_Addr+5,&(Message->Data[0]),Message->DLC);		//读取数据
}
...
void MTDCCU_CAN3_MCP2515_Write_CAN_ID(const unsigned char TxIdReg_Addr, const unsigned char IDE_ExtFlag,const unsigned int can_id)
{
		...
		MTDCCU_CAN3_MCP2515_Set_RegisterS(TxIdReg_Addr,IdBufData,4);		//将id写入寄存器
}
...
whh 2016-12-13
*/
void OpenCloseCan3Intrrupt(FunctionalState arg)
{
	EXTI_InitTypeDef   EXTI_InitStructure;
  ...
  EXTI_InitStructure.EXTI_Mode = EXTI_Mode_Interrupt;          //配置中断模式
  EXTI_InitStructure.EXTI_Trigger = EXTI_Trigger_Falling;      //配置为下降沿触发
  EXTI_InitStructure.EXTI_LineCmd = arg;                  		 //配置中断线使能
  EXTI_Init(&EXTI_InitStructure);        
}
...
void MTDCCU_CAN3_MCP2515_Write_CAN_Message(const unsigned char TxBuf_SIDH_Addr, const CanTxMsg* Message)
{
		...
		MTDCCU_CAN3_MCP2515_Set_RegisterS(TxReg_Addr+5,&(Message->Data[0]),Dlc);		//向寄存器写入数据
		
		if(Message->IDE == CAN_ID_EXT)	//扩展帧
		{
				MTDCCU_CAN3_MCP2515_Write_CAN_ID(TxReg_Addr,Message->IDE,Message->ExtId);		//写入canid
		}
		else	//标准帧
		{
		...
		MTDCCU_CAN3_MCP2515_Set_Register((TxReg_Addr+4),Dlc);		//写入RTR和DLC
	
		//OpenCloseCan3Intrrupt(DISABLE); //whh 关闭can3中断
		MTDCCU_CAN3_MCP2515_Set_Register(TxBuf_SIDH_Addr-1,TXREQ );	//请求发送  //WHH
		//while((MTDCCU_CAN3_MCP2515_Read_Register(TxBuf_SIDH_Addr-1) & TXREQ) == TXREQ);  //等待发送完毕 		
		//OpenCloseCan3Intrrupt(ENABLE);  //whh 使能can3中断
}		
...
unsigned char MTDCCU_CAN3_MCP2515_Get_Free_TxBuf(unsigned char * TxBufNum)
{
		...
		*TxBufNum = 0x00;
		...
		for(i=0;i<3;i++)
		{
			ControlRegValue = MTDCCU_CAN3_MCP2515_Read_Register(ControlReg[i]);		//读取控制寄存器的值
...
void MTDCCU_CAN3_MCP2515_Init_CAN_Buffer(void)
{
		...
		MTDCCU_CAN3_MCP2515_Write_CAN_ID(RXM0SIDH,0,0);	//初始化接受屏蔽寄存器0，为0，不屏蔽id
		MTDCCU_CAN3_MCP2515_Write_CAN_ID(RXM1SIDH,0,0);	//初始化接受屏蔽寄存器1
...
void MTDCCU_CAN3_MCP2515_IntError_Init(const unsigned int  BaudRate)
{
	...
	do{
		...
		//MTDCCU_CAN3_MCP2515_Reset();		//发送复位指令软件复位MCP2515
		MTDCCU_SPI1_CS_LOW();										/* 片选口线置低选中  */
		MTDCCU_SPI1_SendByte(SPICAN_RESET);			//发送寄存器复位命令
		MTDCCU_SPI1_CS_HIGH();									/* 片选口线置高不选中 */
		delay_us(10);
		res = MTDCCU_CAN3_MCP2515_Read_Register(CANSTAT) & REQOP_CONFIG ;
		debugprintf("bbvbvb!\r\n");
	...
	res = MTDCCU_CAN3_MCP2515_Config_Rate(BaudRate);		//设置波特率
	if(res == MCP2515_OK)
	{
		//MTDCCU_CAN3_MCP2515_Init_CAN_Buffer();  //清空屏蔽，滤波，发送，接受寄存器
		// TODO: check why this is needed to receive extended and standard frames
		...
		MTDCCU_CAN3_MCP2515_Set_Register(CANCTRL,REQOP_NORMAL); 	//设置为正常模式
	do{
		res = MTDCCU_CAN3_MCP2515_Read_Register(CANSTAT) & REQOP_NORMAL ;
	}while( res != REQOP_NORMAL);
}
...
/*whh 2016-12-13*/
/*定时器的定义*/
extern OS_TMR Can3ErrResetTmr;	//can3错误中断复位定时器
static u8 s_can3ErrFlag = 0 ;
void Can3ErrResetTmrTrmCallBack(void *p_tmr, void *p_arg)
{
	printf("can3 Err Init Call Back!\r\n ");
	MTDCCU_CAN3_MCP2515_Init(MCP2515_CAN_BAUD_500K);
	s_can3ErrFlag =0;
}
...
void Pro_CAN3_ERROR(void)
{
	...
	EflgState=MTDCCU_CAN3_MCP2515_Read_Register( EFLG ); // 读错误标志寄存器，判断错误类型
	if( EflgState & EWARN )	 // 错误警告寄存器，当TEC或REC大于或等于96时置1
	{
		debugprintf("EWARN Err\r\n");
		 MTDCCU_CAN3_MCP2515_Set_Register(TEC, 0);
		 MTDCCU_CAN3_MCP2515_Set_Register(REC, 0);
	}
	if( EflgState & RXWAR )		  // 当REC大于或等于96时置1
	{debugprintf("RXWAR Error!\r\n");;}
	if( EflgState & TXWAR )		  // 当TEC大于或等于96时置1
	{debugprintf("TXWAR Error!\r\n");;}
	if( EflgState & RXEP )		  // 当REC大于或等于128时置1
	{
		debugprintf("RXEP Error!\r\n");
		//MTDCCU_CAN3_MCP2515_Set_Register(REC, 0);
	}
	if( EflgState & TXEP )		  // 当TEC大于或等于128时置1
	{
		debugprintf("TXEP Error!\r\n");
		//MTDCCU_CAN3_MCP2515_Set_Register(TEC, 0);
	}
	if( EflgState & TXBO )      // 当TEC大于或等于255时置1
	{
		debugprintf("MCP2515 Colse CanBus!\r\n");
		MTDCCU_CAN3_MCP2515_IntError_Init(MCP2515_CAN_BAUD_500K);//复位CAN3  //W HH
		/*开始定时器*/
	}   
	if( EflgState & RX0OVR )		      // 接收缓冲区0溢出
	{
		 debugprintf("Can3 Receive Buffer 0 Overflow!\r\n");
		 MTDCCU_CAN3_MCP2515_Register_BitModify( EFLG, RX0OVR,0 ); 
		// 清中断标志，根据实际情况处理，一种处理办法是发送远程桢，请求数据重新发送
	}
	if( EflgState & RX1OVR )			 // 接收缓冲区1溢出
	{
		debugprintf("Can3 Receive Buffer 1 Overflow!\r\n");
		MTDCCU_CAN3_MCP2515_Register_BitModify( EFLG, RX1OVR,0 ); // 清中断标志;
	}	
	
	MTDCCU_CAN3_MCP2515_Set_Register(EFLG,0x00);	//WHH 清除EFLG寄存器
}	
...
{		
	...
	while(1)
	{
		...
		else if(state & MERRF)  //报文错误标志位
		{
			printf("MERRF Error Interrupt！\r\n");
							//MTDCCU_CAN3_MCP2515_IntError_Init(MCP2515_CAN_BAUD_500K);	//复位CAN3 /*WHH*/
			MTDCCU_CAN3_MCP2515_Register_BitModify(CANINTF,MERRF,0);	// 清报文错误中断标志
		}
		
		else if(state & WAKIF) // 唤醒中断
		{		
			//printf("WAKIF Error Interrupt！\r\n");
	
			MTDCCU_CAN3_MCP2515_SetControlRegister_Mode(REQOP_NORMAL);		// 唤醒后，在仅监听模式，须设置进入正常工作模式
			do{
				SetMode = MTDCCU_CAN3_MCP2515_Read_Register(CANSTAT) & REQOP_NORMAL ;
			}while( SetMode != REQOP_NORMAL);
	
			MTDCCU_CAN3_MCP2515_Register_BitModify(CANINTF,WAKIF,0);	// 清唤醒中断标志
		}
		
		else if(state & ERRIF) // 错误中断
		{
			//printf("ERRIF Error Interrupt！\r\n");
			Pro_CAN3_ERROR( );   //分别处理各个错误
			MTDCCU_CAN3_MCP2515_Register_BitModify(CANINTF,ERRIF,0);	// 清错误中断标志
						//MTDCCU_CAN3_MCP2515_IntError_Init(MCP2515_CAN_BAUD_500K);	//复位CAN3 whh
		}

		else if(state & TX2IF)  // 发送2成功中断
		{
			//debugprintf("33333！\r\n");
			//debugprintf("Tx2 Success Interrupt!\r\n");
			MTDCCU_CAN3_MCP2515_Register_BitModify( CANINTF, TX2IF,0 ); // 清中断标志
		}
		else if(state & TX1IF)  // 发送1成功中断
		{
			//debugprintf("44444！\r\n");
			//debugprintf("Tx1 Success Interrupt!\r\n");
			MTDCCU_CAN3_MCP2515_Register_BitModify( CANINTF, TX1IF,0 );// 清中断标志
		}
		else if(state & TX0IF)    // 发送0成功中断
		{
			
			//debugprintf("55555！\r\n");
			//debugprintf("Tx0 Success Interrupt!\r\n");
			MTDCCU_CAN3_MCP2515_Register_BitModify(CANINTF, TX0IF,0);  // 清中断标志
		}		
		...
		else if(state & RX0IF) 
		{
			...
			//debugprintf("666！\r\n");
			//Msg in Buffer 0
			//debugprintf("Rx0 Interrupt!\r\n");
			MTDCCU_CAN3_MCP2515_Read_CAN_Message(RXB0SIDH,msg);					//读取接收到的信息
			MTDCCU_CAN3_MCP2515_Register_BitModify(CANINTF,RX0IF,0);		//清除中断满标志位
			res= CAN_OK;
		}
		...
		else if(state & RX1IF)
		{
			...
			debugprintf("777！\r\n");
			// Msg in Buffer 1
			//debugprintf("Rx1 Interrupt!\r\n");
			MTDCCU_CAN3_MCP2515_Read_CAN_Message(RXB1SIDH,msg);
			MTDCCU_CAN3_MCP2515_Register_BitModify(CANINTF,RX1IF,0);		//清除中断满标志位
			res= CAN_OK;
		}
		else
		{
	...
	//	/*WHH 重新设置中断，使能中断*/
	//	MTDCCU_CAN3_MCP2515_Set_Register(CANINTE,0);		//关闭所有中断
	//	MTDCCU_CAN3_MCP2515_Register_BitModify(CANINTE,ERRIE | RX0IE | RX1IE,ERRIE_ENABLED | G_RXIE_ENABLED);  //0,1全部接受中断使能  //使能ERRIE：错误中断使能位\报文接收中断RX0IE\RX1IE
	//	MTDCCU_CAN3_MCP2515_Set_Register(CANINTF,0x00);
	//	MTDCCU_CAN3_MCP2515_Set_Register(EFLG,0x00);
